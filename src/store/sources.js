import { reqgetToolsDetail,reqgetComponentGallery,reqgetViewDesign,reqChangeLiked,reqbrowsed,reqartbrowsed} from "@/api/index"
const actions={
    //获取浏览量数据
    async getbrowsed({commit}){
        let result=await reqbrowsed()
        if(result.code==200){//等于200请求成功 返回请求到的数据 提交给mutations
    ...
    async getViewDesign({commit}){
        ...
        async ChangeLiked({commit},{_id,id}){
            ...
            if(result.code==200){//提交给mutations
            console.log('点赞接口成功');
            }
            },
}
